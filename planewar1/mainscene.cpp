...
void MainScene::paintEvent(QPaintEvent *)
{
    ...
    //绘制地图
    painter.drawPixmap(map.map1_posX, 0, map.map1);
    painter.drawPixmap(map.map2_posX, 0, map.map2);
    //绘制英雄飞机
    ...
    // 在右上角绘制
    painter.drawText(GAME_WIDTH - 150, 50, scoreText);
}
...
void MainScene::enemyToScene()
{
    ...
    for(int i=0;i<ENEMY_NUM;i++){
        if(enemys[i].free){//如果为空闲
            ...
            enemys[i].free=false;
            //设置坐标
            enemys[i].x = GAME_WIDTH; // 从屏幕最右边出现
            enemys[i].y = rand() % (GAME_HEIGHT - enemys[i].rect.height()); // 随机高度出现
            break;
        }
...
void MainScene::collisionDetection()
{
    ...
    for (int i = 0; i < ENEMY_NUM; ++i) {
        ...
        if (hero.m_rect.intersects(enemys[i].rect)) {
            ...
            // 弹出失败提示框
            QMessageBox::information(this, "游戏失败", "黄昏见证虔诚的信徒！", QMessageBox::Ok);
